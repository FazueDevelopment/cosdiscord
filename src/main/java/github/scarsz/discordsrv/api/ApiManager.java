...
package github.fazuedevelopment.cosdiscord.api;

import com.google.common.collect.Sets;
import com.hrakaroo.glob.GlobPattern;
import github.fazuedevelopment.cosdiscord.DiscordSRV;
import github.fazuedevelopment.cosdiscord.api.commands.*;
import github.fazuedevelopment.cosdiscord.api.events.Event;
import github.fazuedevelopment.cosdiscord.api.events.GuildSlashCommandUpdateEvent;
...
public class ApiManager extends ListenerAdapter {
    ...
    public void subscribe(Object listener) {
        ...
        if (methodsAnnotatedSubscribe == 0) throw new IllegalArgumentException(listener.getClass().getName() + " attempted DiscordSRV API registration but no public methods inside of it were annotated @Subscribe (github.fazuedevelopment.cosdiscord.api.Subscribe)");

        if (!listener.getClass().getPackage().getName().contains("cosdiscord.discordsrv")) {
    ...
    public void updateSlashCommands() {
        ...
        RestAction.allOf(guildCommandUpdateActions).queue(all -> {
            ...
            if (errors.stream().anyMatch(r -> r.getException() instanceof ErrorResponseException && ((ErrorResponseException) r.getException()).getErrorResponse() == ErrorResponse.MISSING_ACCESS)) {
                String invite = "https://cosdiscord.me/authorize#";
                try {
